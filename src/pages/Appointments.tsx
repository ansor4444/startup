          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/50 text-xs text-muted-foreground">
                <tr>
                  <th className="text-left p-3">{t("field.date")} / {t("field.time")}</th>
                  <th className="text-left p-3">{t("role.client")}</th>
                  <th className="text-left p-3">{t("nav.services")}</th>
                  <th className="text-left p-3">{t("role.barber")}</th>
                  <th className="text-left p-3">{t("field.price")}</th>
                  <th className="text-left p-3">{t("field.status")}</th>
                  {(isAdmin || isBarber) && <th className="text-left p-3">{t("field.action")}</th>}
                </tr>
              </thead>
              <tbody>
                {items.map((a) => (
                  <tr key={a.id} className="border-t border-border">
                    ...
                    <td className="p-3">{Number(a.total_price).toLocaleString()} {t("common.sum")}</td>
                      ...
                      <td className="p-3">
                        <div className="flex gap-1">
                          <select value={a.status} onChange={(e) => setStatus(a.id, e.target.value)} className="text-xs border border-border rounded px-1.5 py-1 bg-background">
                            <option value="pending">{t("status.pending")}</option><option value="confirmed">{t("status.confirmed")}</option><option value="completed">{t("status.completed")}</option><option value="cancelled">{t("status.cancelled")}</option><option value="no_show">{t("status.no_show")}</option>
                          </select>
