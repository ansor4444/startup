...
export default function Book() {
  ...
  useEffect(() => {
    if (!barberId || !date) { setSchedule(null); return; }
    const dayOfWeek = new Date(`${date}T00:00:00`).getDay();
    supabase.from("barber_schedule").select("*")
  ...
  useEffect(() => {
    ...
    const fetchTaken = async () => {
      const { data, error } = await supabase.rpc("get_barber_taken_slots", {
        _barber_id: barberId,
        _appointment_date: date,
      });
      if (error) {
        const { data: fallback } = await supabase.from("appointments").select("appointment_time")
          .eq("barber_id", barberId).eq("appointment_date", date).neq("status", "cancelled");
        setTaken((fallback || []).map((a) => a.appointment_time?.slice(0, 5)));
        return;
      }
  ...
  const isOutsideSchedule = (slotTime: string) => {
    if (!schedule) return false;
    if (schedule.is_off) return true;
    const slot = toMinutes(slotTime);
    const start = toMinutes(schedule.start_time || "09:00");
    const end = toMinutes(schedule.end_time || "20:00");
    ...
    const isBreak = breakStart !== null && breakEnd !== null && slot >= breakStart && slot < breakEnd;
    return slot < start || slot >= end || isBreak;
  };

  const isSlotDisabled = (slotTime: string) => {
    return taken.includes(slotTime) || isPastSlot(date, slotTime) || isOutsideSchedule(slotTime);
  };

  useEffect(() => {
    if (time && isSlotDisabled(time)) setTime("");
  }, [time, taken, date, schedule]);

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !serviceId || !barberId || !time) return toast.error("Barcha maydonlarni to'ldiring");
    if (isSlotDisabled(time)) return toast.error("Bu vaqt band yoki o'tib ketgan");
    setLoading(true);
    ...
    const { data: latestTaken } = await supabase.rpc("get_barber_taken_slots", {
      _barber_id: barberId,
      _appointment_date: date,
    });
    if ((latestTaken || []).some((slot) => slot.appointment_time?.slice(0, 5) === time)) {
      setTaken((latestTaken || []).map((slot) => slot.appointment_time?.slice(0, 5)));
      setLoading(false);
      return toast.error("Bu vaqt allaqachon band qilingan");
    }
    ...
    setLoading(false);
    if (error) {
      if ((error as any).code === "23505") return toast.error("Bu vaqt allaqachon band qilingan");
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" required value={date} min={formatLocalDate(new Date())} onChange={(e) => setDate(e.target.value)} className="w-full px-3 py-2 text-sm border border-border rounded-md bg-background" />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-1.5">
              {TIMES.map((tm) => {
                ...
                const isPast = isPastSlot(date, tm);
                const isClosed = isOutsideSchedule(tm);
                const disabled = isTaken || isPast || isClosed;
                return (
                  <button type="button" key={tm} disabled={disabled} onClick={() => setTime(tm)}
                    title={isTaken ? "Band qilingan" : isPast ? "Vaqt o'tib ketgan" : isClosed ? "Ish vaqti emas" : undefined}
