...
export default function BarberStatsPage() {
  ...
  const stats = useMemo(() => {
    ...
    completed.forEach((a) => { const n = a.services?.name || t("common.unknown"); svcMap[n] = (svcMap[n] || 0) + 1; });
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <PageHeader title={t("nav.statistics")} subtitle={t("dash.barber.statsSubtitle")} />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Stat label={t("nav.appointments")} value={stats.totalAppts} hint={`${stats.completed} ${t("dash.barber.completed")}`} />
        <Stat label={t("dash.admin.revenue")} value={`${(stats.totalRevenue / 1000).toFixed(0)}K ${t("common.sum")}`} />
        <Stat label={t("dash.barber.activeClients")} value={stats.activeClients} />
        <Stat label={t("dash.barber.rating")} value={rating.toFixed(1)} hint={`${reviewCount} ${t("dash.barber.reviews")}`} />
      </div>

      <FeatureGate feature="advanced_stats" audience="barber">
        <div className="grid lg:grid-cols-2 gap-4">
          <Card>
            <h3 className="text-sm font-semibold mb-4">{t("dash.barber.dailyRevenue30")}</h3>
            <div className="h-64">
              <ResponsiveContainer>
          ...
          <Card>
            <h3 className="text-sm font-semibold mb-4">{t("dash.barber.topServices")}</h3>
            {!stats.topServices.length ? <Empty text={t("common.noData")} /> : (
              <div className="h-64">
                <ResponsiveContainer>
