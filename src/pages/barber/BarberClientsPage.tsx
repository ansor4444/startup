...
export default function BarberClientsPage() {
  ...
  const addBlacklist = async (clientId: string) => {
    ...
    toast.success(t("dash.barber.added"));
    load(barberId!);
  };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <PageHeader title={t("nav.clients")} subtitle={t("dash.barber.clientsSubtitle")} />

      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab("all")} className={`px-3 py-1.5 text-xs rounded-md ${tab === "all" ? "bg-primary text-primary-foreground" : "bg-secondary"}`}>{t("nav.clients")} ({clients.length})</button>
        <button onClick={() => setTab("blacklist")} className={`px-3 py-1.5 text-xs rounded-md ${tab === "blacklist" ? "bg-primary text-primary-foreground" : "bg-secondary"}`}>{t("dash.barber.blacklistTab")} ({blacklist.length})</button>
      </div>

      {tab === "all" ? (
        !clients.length ? <Empty text={t("common.empty")} /> : (
          <div className="space-y-2">
              ...
              return (
                <Card key={c.client_id}>
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="font-medium text-foreground">{profiles[c.client_id]?.full_name || c.name || t("role.client")} {isBl && <span className="text-[10px] px-1.5 py-0.5 bg-destructive/10 text-destructive rounded ml-2">{t("dash.barber.blacklistTab")}</span>}</p>
                      <p className="text-xs text-muted-foreground">{c.visits} {t("dash.barber.visits")} • {c.totalSpent.toLocaleString()} {t("common.sum")} • {t("dash.barber.lastVisit")} {c.lastVisit}</p>
            ...
            {!can("unlimited_clients") && clients.length > FREE_LIMIT && (
              <Card className="border-dashed border-2 text-center">
                ...
                <p className="text-sm font-semibold mb-1">+{clients.length - FREE_LIMIT} {t("dash.barber.hiddenClients")}</p>
                <p className="text-xs text-muted-foreground mb-3">{t("dash.barber.freePlanLimit").replace("{limit}", FREE_LIMIT.toString())}</p>
                <Link to="/plans" className="inline-block px-4 py-2 text-xs bg-primary text-primary-foreground rounded-md">{t("plans.getPro")}</Link>
              </Card>
            )}
          </div>
        )
      ) : (
        !blacklist.length ? <Empty text={t("dash.barber.blacklistEmpty")} /> : (
          <div className="space-y-2">
            {blacklist.map((b) => (
              <Card key={b.id}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium">{profiles[b.client_id]?.full_name || t("role.client")}</p>
                    {b.reason && <p className="text-xs text-muted-foreground">{t("dash.barber.reason")} {b.reason}</p>}
                  </div>
