...
export default function BarberPortfolioPage() {
  ...
  const handleUpload = async (file: File, caption: string = "") => {
    ...
    try {
      if (mode === "before_after") {
        if (!beforeFile) { setUploading(false); return toast.error(t("dash.barber.selectBeforeFirst")); }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <PageHeader title={t("nav.portfolio")} subtitle={t("dash.barber.portfolioSubtitle")} />
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {(["image", "video", "before_after"] as Mode[]).map((m) => (
            <button key={m} onClick={() => setMode(m)} className={`px-3 py-1.5 text-xs rounded-md ${mode === m ? "bg-primary text-primary-foreground" : "bg-secondary"}`}>
              {m === "image" ? t("dash.barber.image") : m === "video" ? t("dash.barber.video") : t("dash.barber.beforeAfter")}
            </button>
          ))}
        </div>
        ...
        {mode === "before_after" ? (
          <div className="flex flex-wrap gap-2 items-center">
            <label className="px-3 py-2 text-xs bg-secondary rounded-md cursor-pointer flex items-center gap-1.5">
              <Upload className="w-3.5 h-3.5" /> {beforeFile ? `${t("dash.barber.before")}: ${beforeFile.name}` : t("dash.barber.selectBefore1")}
            ...
            <label className={`px-3 py-2 text-xs rounded-md flex items-center gap-1.5 ${beforeFile ? "bg-primary text-primary-foreground cursor-pointer" : "bg-muted text-muted-foreground cursor-not-allowed"}`}>
              <Upload className="w-3.5 h-3.5" /> {t("dash.barber.selectAfter2")}
          ...
          <label className="inline-flex items-center gap-1.5 px-3 py-2 text-xs bg-primary text-primary-foreground rounded-md cursor-pointer">
            <Upload className="w-3.5 h-3.5" /> {uploading ? "..." : (mode === "video" ? t("dash.barber.uploadVideo") : t("dash.barber.uploadImage"))}
            <input type="file" accept={mode === "video" ? "video/*" : "image/*"} hidden onChange={(e) => e.target.files?.[0] && handleUpload(e.target.files[0])} />
          </label>
        )}
      </Card>

      {!items.length ? <Empty text={t("dash.barber.noPortfolioYet")} /> : (
          ...
          <div className="max-w-4xl max-h-full" onClick={(e) => e.stopPropagation()}>
            {lightbox.media_type === "video" ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><p className="text-white text-xs mb-1">{t("dash.barber.before")}</p><img src={lightbox.before_url} className="rounded-lg max-h-[70vh]" /></div>
                <div><p className="text-white text-xs mb-1">{t("dash.barber.after")}</p><img src={lightbox.after_url} className="rounded-lg max-h-[70vh]" /></div>
              </div>
            ) : (
