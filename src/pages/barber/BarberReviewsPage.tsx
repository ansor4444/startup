...
export default function BarberReviewsPage() {
  ...
  const sendReply = async (id: string) => {
    ...
    toast.success(t("dash.barber.replySaved"));
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <PageHeader title={t("nav.reviews")} subtitle={t("dash.barber.reviewsSubtitle")} />
      {!reviews.length ? <Empty text={t("dash.barber.noReviewsYet")} /> : (
            ...
            <Card key={r.id}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-medium">{profiles[r.client_id]?.full_name || t("role.client")}</span>
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <Star key={n} className={`w-3.5 h-3.5 ${n <= r.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`} />
                      ))}
                    </div>
                    <span className="text-[10px] text-muted-foreground">{new Date(r.created_at).toLocaleDateString(lang === 'uz' ? 'uz-UZ' : lang === 'ru' ? 'ru-RU' : 'en-US')}</span>
                  </div>
              ...
              {r.reply ? (
                <div className="mt-3 ml-4 p-2.5 bg-primary/5 border-l-2 border-primary rounded">
                  <p className="text-[10px] text-primary font-medium mb-0.5 flex items-center gap-1"><MessageCircle className="w-3 h-3" /> {t("dash.barber.yourReply")} • {new Date(r.replied_at).toLocaleDateString(lang === 'uz' ? 'uz-UZ' : lang === 'ru' ? 'ru-RU' : 'en-US')}</p>
                  <p className="text-sm text-foreground">{r.reply}</p>
                </div>
              ) : (
                <div className="mt-3 flex gap-2">
                  <input
                    placeholder={t("dash.barber.writeReply")}
                  ...
                  <button onClick={() => sendReply(r.id)} className="px-3 py-1.5 text-xs bg-primary text-primary-foreground rounded flex items-center gap-1">
                    <Send className="w-3 h-3" /> {t("action.send")}
                  </button>
                </div>
              )}
            </Card>
