  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Stat label={t("dash.admin.todayRevenue")} value={`${todaySum.toLocaleString()} ${t("common.sum")}`} />
        <Stat label={t("dash.admin.monthRevenue")} value={`${monthSum.toLocaleString()} ${t("common.sum")}`} />
        <Stat label={t("dash.admin.totalRecords")} value={items.length} />
      </div>
        ...
        <Card className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="text-left p-3">{t("field.date")}</th><th className="text-left p-3">{t("role.client")}</th><th className="text-left p-3">{t("nav.services")}</th><th className="text-left p-3">{t("role.barber")}</th><th className="text-left p-3">{t("field.method")}</th><th className="text-left p-3">{t("field.amount")}</th></tr></thead>
              <tbody>
                {items.map((p) => (
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="p-3 font-semibold text-primary">{Number(p.amount).toLocaleString()} {t("common.sum")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
