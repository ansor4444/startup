                ...
                return (
                  <li key={th.id}>
                    <button onClick={() => setActive(th)}
                      ...
                      <p className="text-xs text-muted-foreground">{new Date(th.last_message_at).toLocaleString(lang === 'uz' ? 'uz-UZ' : lang === 'ru' ? 'ru-RU' : 'en-US')}</p>
                    </button>
                  </li>
                );
              })}
                ...
                {messages.map((m) => (
                  <div key={m.id} className={`flex ${m.sender_id === user?.id ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[75%] px-3 py-2 rounded-lg text-sm ${m.sender_id === user?.id ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>
                      {m.body}
                      <p className="text-[10px] opacity-60 mt-0.5">{new Date(m.created_at).toLocaleTimeString(lang === 'uz' ? 'uz-UZ' : lang === 'ru' ? 'ru-RU' : 'en-US', { hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                  </div>
                ))}
                <div ref={endRef} />
