...
export default function Plans() {
  ...
  const subscribe = async (p: any) => {
    ...
    toast.success(`${p.name} ${t("plans.activated")}`); load();
  };
                ...
                <ul className="grid sm:grid-cols-2 gap-x-4 gap-y-1.5 mb-5">
                  {features.map((f, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                      <Check className="w-3.5 h-3.5 mt-0.5 text-primary flex-shrink-0" /> <span>{t(f)}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {tierPlans.map((p) => {
                    ...
                    return (
                      ...
                      >
                        <p className="text-xs text-muted-foreground">{t(p.duration_label)}</p>
                        <p className="text-sm font-bold text-foreground mt-0.5">
                          {p.price > 0 ? `${Number(p.price).toLocaleString()} ${t("common.sum")}` : t("plans.free")}
                        </p>
