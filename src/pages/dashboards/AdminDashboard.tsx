...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <PageHeader title={t("dash.admin.title")} subtitle={t("dash.admin.subtitle")} />

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Stat label={t("dash.admin.todayBookings")} value={stats.today} />
        <Stat label={t("dash.admin.todayRevenue")} value={`${stats.todayRevenue.toLocaleString()} ${t("common.sum")}`} />
        <Stat label={t("dash.admin.monthRevenue")} value={`${stats.monthRevenue.toLocaleString()} ${t("common.sum")}`} />
        <Stat label={t("dash.admin.totalClients")} value={stats.clients} />
      </div>

      <Card>
        <h3 className="text-sm font-semibold text-foreground mb-4">{t("dash.admin.upcoming")}</h3>
          ...
          <div className="space-y-2">
            {upcoming.map((a) => (
              <div key={a.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-md">
                <div>
                  <p className="text-sm font-medium text-foreground">{a.client_name || t("role.client")} → {a.barbers?.full_name}</p>
