...
export default function BarberDashboard() {
  ...
  if (!barberId) {
    return <div className="p-6"><Empty text={t("dash.barber.noProfile")} /></div>;
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <Stat label={t("dash.admin.todayRevenue")} value={`${todayRevenue.toLocaleString()} ${t("common.sum")}`} />
      </div>
          ...
          <div className="space-y-2">
            {appts.map((a) => (
              <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 p-3 bg-muted/30 rounded-md">
                <div>
                  <p className="text-sm font-medium text-foreground">{a.appointment_time?.slice(0, 5)} — {a.client_name || t("role.client")}</p>
                  <p className="text-xs text-muted-foreground">{a.services?.name} • {Number(a.total_price || a.services?.price || 0).toLocaleString()} {t("common.sum")}</p>
                </div>
                <div className="flex gap-2">
                  <span className="text-xs px-2 py-1 rounded bg-primary/10 text-primary">{t(`status.${a.status}`)}</span>
                  {a.status !== "completed" && (
                    <>
                      <button onClick={() => updateStatus(a.id, "confirmed")} className="text-xs px-2 py-1 bg-secondary rounded hover:opacity-90">{t("action.confirm")}</button>
